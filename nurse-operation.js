...
function fetchpatientRecords(searchTerm = "") {
  $.ajax({
    url: "backend-all-operation-record.php",
    type: "GET",
    dataType: "json",
    success: function (data) {
      var html = "";
        ...
        if (
          ...
          html += `
                                      <tr class="text-nowrap">
                                          ...
                                          <td class="text-nowrap">${patientRecord.operationType}&deg;C</td>
                                          <td class="text-nowrap">${patientRecord.record}</td>
      ...
      $(".delete-patientRecord").click(function () {
        ...
        if (confirmDelete) {
          deletepatientRecord("backend-nurse-delete-operation-record.php", id);
...
$(document).ready(function() {
    $('#myForm').submit(function(e) {
        ...
        $.ajax({
            url: 'backend-nurse-create-operation-record.php',
            type: 'POST',
            data: formData,
            success: function(data) {
                $('#result').html(data);
