  ...
  $.ajax({
    ...
    success: function (data) {
        ...
        if (
          ...
          html += `
                                      <tr class="text-nowrap">
                                          ...
                                          <td class="text-nowrap"><button data-id="${pharmacy.id}" data-patientname="${pharmacy.patientName}" class="delete-pharmacy btn btn-danger btn-sm">Delete</button></td>
                                      </tr>
                              `;
      ...
      $(".delete-pharmacy").click(function () {
        ...
        var confirmDelete = confirm(
          "Are you sure you want to delete pharmacy with " +
            patientName + " ?"
        );

        if (confirmDelete) {
          deletepharmacy("backend-pharmacist-delete-pharmacy.php", id);
...
$(document).ready(function() {
    $('#myForm').submit(function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: 'backend-pharmacist-provide-medication.php',
            data: $(this).serialize(),
